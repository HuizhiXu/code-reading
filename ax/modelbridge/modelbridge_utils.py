...
它将风险度量（risk measure）的名称映射到相应的类。
这个字典是在多目标优化和决策制定中使用的，风险度量是用来量化和管理不确定性下的风险的一种方法。
...
RISK_MEASURE_NAME_TO_CLASS: Dict[str, Type[RiskMeasureMCObjective]] = {
    ...
    "CVaR": CVaR,  # 用于量化超过某个阈值的潜在损失的平均大小
    "MARS": MARS,
    "MVaR": MVaR,
...
def extract_risk_measure(risk_measure: RiskMeasure) -> RiskMeasureMCObjective:
    ...
    从Ax RiskMeasure 对象中提取出对应的 BoTorch 风险度量目标对象
...
def extract_parameter_constraints(
    ...
    """给定的参数约束列表中提取线性约束，并返回这些约束的矩阵和向量表示
        函数返回的 (A, b) 可以用来定义线性约束 Ax ≤ b
    """
...
def extract_search_space_digest(
    ...
    从一个搜索空间对象中提取基本的参数属性，并返回一个 SearchSpaceDigest 对象。
    这个对象包含了搜索空间的摘要信息，如参数的界限、分类特征、序数特征等


    如果参数是 ChoiceParameter 类型，它会根据参数是否是任务参数、是否有序，将其索引添加到相应的列表中。
    同时，将参数的选择列表添加到 discrete_choices 字典中，并将选择的最小值和最大值作为界限添加到 bounds 列表中。

    如果参数是 RangeParameter 类型，并且不是以对数尺度表示，函数会检查参数的类型。
    如果是整数类型，将其索引添加到 ordinal_features 列表中，并将整数范围内的值作为选择列表添加到 discrete_choices 字典中。对于非整数类型，直接将参数的上下界添加到 bounds 列表中。
    
    如果参数是保真度类型，函数会检查其目标值是否为数值类型，如果是，则将其添加到 target_fidelities 字典中，并将参数的索引添加到 fidelity_features 列表中。
...
def extract_robust_digest(
    ...
    用于从给定的搜索空间中提取出 RobustSearchSpaceDigest 对象，这个对象包含了关于鲁棒搜索空间的摘要信息。
    这个信息可以用于理解和处理搜索空间中的分布参数和环境变量，特别是在考虑参数扰动和环境变化对优化结果的影响时。
...
def extract_objective_weights(objective: Objective, outcomes: List[str]) -> np.ndarray:
    ...
    用于从优化问题的 Objective 对象中提取目标权重。
    这些权重用于指导优化算法在多目标或单目标优化问题中如何平衡不同目标的重要性。
    权重的正负表示目标是应该最大化还是最小化。
...
def extract_outcome_constraints(
    ...
    """
    从给定的 OutcomeConstraint 对象列表中提取目标约束，并以线性约束矩阵和向量的形式返回这些约束。
    函数返回的 (A, b) 可以用来定义线性约束 Ax ≤ b
    """
...
def validate_and_apply_final_transform(
    ...
    """
    对优化问题中的各种参数进行验证，并将它们转换（应用）为特定的数据类型
    """
...
def get_fixed_features(
    ...
    用于将 fixed_features（固定特征）从 ObservationFeatures 对象格式转换为字典格式
...
def pending_observations_as_array_list(
    ...
    将待处理的观测数据（pending_observations）重新格式化为一个 NumPy 数组的列
...
def parse_observation_features(
    ...
    将原始的模型生成的候选解（以 NumPy 数组 X 的形式给出）转换为 ObservationFeatures 对象的列表
    Args:
...
def get_pareto_frontier_and_configs(
    ...
    # checked_cast 用于在运行时对值进行类型检查，并在值的类型不符合预期时抛出异常
...
def hypervolume(
    ...
    """计算基于观测数据的帕累托前沿的超体积（hypervolume）

    Args:
        modelbridge: TorchModelBridge 类型的对象，它保存了之前的训练数据
        observation_features: The observation features for the in-sample arms.
        objective_thresholds: 定义了可以贡献于超体积的超矩形的起点。The objective thresholds to be used for computing
