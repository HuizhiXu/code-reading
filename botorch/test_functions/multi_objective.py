...
class BraninCurrin(MultiObjectiveTestProblem):
    r"""
    这段代码定义了一个名为BraninCurrin的类，它是一个包含Branin函数和Currin函数的双目标问题。该类继承自MultiObjectiveTestProblem类。

    Branin函数是一个在特定区域上定义的二维函数，具体形式如下:
    ...
    其中x为输入变量，x_0和x_1分别表示输入变量的两个维度。
    Branin Function的解释：https://www.sfu.ca/~ssurjano/branin.html

   Currin函数是另一个二维函数，具体形式如下
    ...
    def __init__(self, noise_std: Optional[float] = None, negate: bool = False) -> None:
        ...
        Args:
            ...
            BraninCurrin类的构造函数接受两个参数：noise_std和negate。
            noise_std用于指定观测噪声的标准差，negate用于指定是否对目标函数取反。
    ...
    def evaluate_true(self, X: Tensor) -> Tensor:
        ...
        """
        evaluate_true方法是该类的一个成员方法，用于计算两个目标函数（Branin和Currin）
        在给定输入X上的真实值。该方法首先调用_rescaled_branin方法计算Branin函数的值，
        然后调用_currin方法计算Currin函数的值，最后将两个值以张量的形式返回。
...
class DH(MultiObjectiveTestProblem, ABC):
    ...
    DH的基类，用于表示具有鲁棒多目标优化的DH问题（DH表示Deb and Ho）

    DH问题是一个鲁棒多目标优化问题，其中输入变量经过均值鲁棒性处理，
    并在一个边长为delta_0 = delta，delta_i = 2 * delta, i > 0的盒子中以均匀分布进行扰动。
    其中，delta的取值范围为0.01（对于DH1和DH2问题）或0.03（对于DH3和DH4问题）。

    目标是使两个目标函数都最小化。DH类的构造函数接受以下参数：dim（输入维度），noise_std（观测噪声的标准差），negate（是否对函数取反）。
...
class DTLZ(MultiObjectiveTestProblem):
    ...
    DTLZ问题是具有高维输入和多个目标函数的问题。
    基于输入维度dim和目标数量num_objectives，该类在构造函数中初始化了一些变量，包括dim、
    num_objectives、k（一个用于计算约束条件的变量）、_bounds（输入变量的边界）和_ref_point（参考点）。
...
class GMM(MultiObjectiveTestProblem):
    ...
    定义了一个名为GMM的类，用于表示每个目标都是高斯混合模型（Gaussian Mixture Model）的测试问题
    ...
    def __init__(
        ...
        r"""
        在构造函数中，首先通过检查num_objectives的值，确定目标数量是否在2到4之间。
        然后，根据目标数量设置参考点_ref_point。
        接下来，定义了高斯混合模型的位置(gmm_pos)、协方差(gmm_covar)和归一化因子(gmm_norm)。
        Args:
            noise_std: Standard deviation of the observation noise.
            negate: If True, negate the objectives.
        ...
        # 使用register_buffer方法将这些定义的张量注册为模型的缓冲区，以便在后续的函数计算中使用
    ...
    def evaluate_true(self, X: Tensor) -> Tensor:
        r"""Evaluate the GMMs.
        用于计算GMM问题的真实函数值。在该方法中，使用定义的高斯混合模型，
        计算给定输入X的概率密度函数值，并乘以归一化因子，最后返回负值。
...
class Penicillin(MultiObjectiveTestProblem):
    ...
    Penicillin类实现了一个青霉素产量模拟器，其目标是在最小化发酵时间和CO2副产物的同时最大化青霉素产量。

    参考点（reference point）是根据在一个大的离散随机设计集上帕累托前沿使用infer_reference_point
    启发式方法进行设定的。infer_reference_point是一种启发式方法，用于估计帕累托前沿的参考点。
    它可能根据前沿上的解的分布、密度或其他特性来选择参考点，以便在搜索过程中引导优化算法尽可能接近这个参考点。

    参考点（reference point）是根据在一个大的离散随机设计集上帕累托前沿使用infer_reference_point启发式方法
    进行设定的。infer_reference_point是一种启发式方法，用于估计帕累托前沿的参考点。
    它可能根据前沿上的解的分布、密度或其他特性来选择参考点，以便在搜索过程中引导优化算法尽可能接近这个参考点。
...
class ToyRobust(MultiObjectiveTestProblem):
    ...
    ToyRobust类实现了一个一维问题，其中帕累托前沿对输入噪声非常敏感。
    通过设置参考点来引导优化算法在解空间中的搜索方向。
    参考点是通过infer_reference_point启发式方法在离散随机设计集上的帕累托前沿上设定的。

    具体而言，正常目标函数上的帕累托前沿对输入噪声非常敏感。第一个目标函数是线性函数和正弦函数的混合，
    而第二个目标函数是修改后的Levy函数，其中第二个参数被固定。
...
class ZDT(MultiObjectiveTestProblem):
    ...
    ZDT代表"Zitzler-Deb-Thiele"，是一系列经典的多目标优化测试问题。
    ZDT问题是由Zitzler、Deb和Thiele在2000年提出的，用于评估多目标优化算法的性能。
    这些问题通常包括一组具有不同特征的目标函数，涉及到连续变量和非线性关系。
...
class CarSideImpact(MultiObjectiveTestProblem):
    ...
    CarSideImpact类实现了一个汽车侧面碰撞问题。
    汽车侧面碰撞问题是一个多目标优化问题，旨在通过设计汽车的结构和材料来最小化乘员受伤和车辆损坏。
    优化的目标包括最小化乘员胸部加速度、最小化车辆失效面积和最小化车辆失效能量。

    参考点（reference point）是根据[Tanabe2020]中近似帕累托前沿的理想点（ideal）和最差点（nadir）进行设定的。
    参考点的计算方式是nadir + 0.1 * (ideal - nadir)。参考点的目的是提供一个理想最优解的近似值，
    以引导多目标优化算法在解空间中的搜索方向。

    另外，max_hv是基于[Tanabe2020]中近似帕累托前沿计算得出的。它是一种衡量帕累托前沿覆盖范围的指标，用于评估优化算法的性能。
...
class BNH(MultiObjectiveTestProblem, ConstrainedBaseTestProblem):
    ...
    BNH问题是一个约束多目标优化问题，旨在最小化目标函数并满足一组约束条件。
    它是由GarridoMerchan在2020年提出的，用于评估优化算法的性能。
    BNH类实现了一个约束多目标优化问题，其中目标函数和约束条件的计算公式在evaluate_true和evaluate_slack_true方法中定义。
    ...
    # 定义了参考点（reference point）。参考点是一个用于引导优化算法的向量，用于指示希望在目标空间中达到的最佳解。在这里，参考点的值被设定为[0.0, 0.0]，但是注释中提到需要确定一个合适的参考点。
...
class CONSTR(MultiObjectiveTestProblem, ConstrainedBaseTestProblem):
    ...
    CONSTR问题是一个约束多目标优化问题，旨在最小化目标函数并满足一组约束条件。
    See [GarridoMerchan2020]_ for more details on this problem. Note that this is a
    minimization problem.
    """
...
class DiscBrake(MultiObjectiveTestProblem, ConstrainedBaseTestProblem):
    ...
    定义了一个名为DiscBrake的类，它是一个具有2个目标和4个约束条件的多目标优化问题。主要解释如下
...
class MW7(MultiObjectiveTestProblem, ConstrainedBaseTestProblem):
    r"""The MW7 problem.
    定义了一个名为MW7的类，它是一个具有2个目标和2个约束条件的多目标优化问题。
    约束函数可以和目标函数混在一起吗（这里能实现x和y的约束关系吗）
...
class WeldedBeam(MultiObjectiveTestProblem, ConstrainedBaseTestProblem):
    r"""
    焊接梁问题（Welded Beam）是一个多目标优化问题的测试问题。
