    ...
    **All classes in 'botorch_modular' directory are under
    ...
    def construct(
        ...
        **kwargs: Any,
        ...
        # use_model_list 用来确定是否应该使用模型列表（ModelListGP）来处理
        if use_model_list(
            datasets=datasets,
    ...
    def _construct_model_list(
        ...
        **kwargs: Any,
        ...
        这段代码定义了一个名为 `_construct_model_list` 的私有方法，它属于某个类，这个类可能是用于构建和管理 BoTorch 模型列表（`ModelListGP`）的。`ModelListGP` 是 BoTorch 库中的一个类，用于处理多任务高斯过程模型，其中每个任务对应一个不同的输出。

        参数解释：
        - `datasets`: 一个包含 `SupervisedDataset` 对象的列表，这些对象包含了用于训练 `ModelListGP` 中各个子模型的数据。
        每个数据集对应一个输出结果，并且输出的顺序应与 `metric_names` 参数中度量名称的顺序相匹配。
        - `metric_names`: 一个可迭代的字符串集合，包含度量名称，顺序应与 `datasets` 中的数据集顺序相对应。
        - `search_space_digest`: 一个可选参数，如果 `botorch_submodel_class` 未提供，则必须提供，以便自动选择适当的 BoTorch 子模型类。
        - `**kwargs`: 可变关键字参数，接受以下参数：
        - `fidelity_features`: 表示保真度特征的列索引。
        - `task_features`: 表示任务特征的列索引。
        - `categorical_features`: 表示分类特征的列索引。
        - `robust_digest`: 一个可选的 `RobustSearchSpaceDigest` 对象，如果使用 `RobustSearchSpace`，则携带额外的属性。

    方法逻辑：
    1. 首先，方法检查是否提供了 `botorch_submodel_class` 或 `search_space_digest`。如果没有提供这两个参数中的任何一个，将抛出 `UserInputError` 异常，因为需要这些信息来选择适当的子模型类。

    2. 然后，方法将 `datasets` 保存为类的成员变量 `_training_data`。

    3. 接下来，方法从 `kwargs` 中提取出 `fidelity_features`、`task_feature` 和 `submodel_input_transforms` 等参数。

    4. 方法初始化一个空的 `submodels` 列表，用于存储将要构建的子模型。

    5. 对于 `metric_names` 和 `datasets` 中的每个数据集，方法执行以下操作：
    - 确定子模型的类（如果未提供 `botorch_submodel_class`，则使用 `choose_model_class` 函数选择一个类）。
    - 如果 `_outcomes` 属性已定义，并且度量名称不在其中，则记录一条警告并继续下一个数据集。
    - 使用子模型类构建格式化的模型输入。
    - 设置格式化的输入和其他相关参数（如协方差模块、似然函数、输出转换和输入转换）。
    - 将格式化的模型输入传递给子模型类，创建子模型，并将其添加到 `submodels` 列表中。

    6. 最后，使用 `submodels` 列表中的所有子模型实例创建 `ModelListGP` 实例，并将其保存为类的成员变量 `_model`。

    这个方法的主要作用是根据提供的多个数据集和度量名称构建一个 `ModelListGP` 实例，这个实例可以用于多任务高斯过程回归，其中每个任务对应一个不同的输出。
