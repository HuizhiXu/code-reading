...
class AnalyticAcquisitionFunction(AcquisitionFunction, ABC):
    ...
    def __init__(
        ...
        构造函数接受一个 model 参数，这应该是一个已经拟合好的单输出模型。
        posterior_transform 参数是一个可选的 PosteriorTransform 对象，
        它用于将多输出模型的后验分布转换为单输出后验分布。
        如果模型是多输出的，但没有提供转换，则会抛出一个错误。
...
class LogProbabilityOfImprovement(AnalyticAcquisitionFunction):
    ...
    计算在给定候选点 X 上，目标函数值超过当前最佳观测值的概率的对数。
    计算概率的对数（即 logPI）比直接计算概率本身在数值上更稳定，这有助于优化过程中避免数值问题，特别是在概率非常小或非常大的情况下。
...
class ProbabilityOfImprovement(AnalyticAcquisitionFunction):
    ...
    概率改进: 该获取函数直接计算目标函数值超过当前最佳观测值的概率，而不是对数形式。
...
class qAnalyticProbabilityOfImprovement(AnalyticAcquisitionFunction):
    ...
    计算在给定一批候选点 X 上，目标函数值超过当前最佳观测值的概率的近似值。
    MVNXPB: 这是一种用于近似计算多维正态分布在某个区间内的概率的算法。
    它通过在多维正态分布上进行秩-1 逼近来实现，这种方法在计算上比较高效，但得到的结果是近似的。

    这个获取函数适用于需要在一批候选点上评估概率改进的场景，特别是在目标函数的维度较高时，
    传统的蒙特卡洛方法可能计算成本过高。通过使用 MVNXPB 算法，可以在保持计算效率的同时，
    得到一个可接受的近似结果。
...
class ExpectedImprovement(AnalyticAcquisitionFunction):
    ...
    计算在给定单个测试点 x 上，相对于当前最佳观测值的预期改进（Expected Improvement，EI）

    虽然 ExpectedImprovement 类提供了一个解析型的EI计算方法，但在实践中，
    通常推荐使用 LogExpectedImprovement 类。
    这是因为 LogExpectedImprovement 类通过取对数来解决梯度消失问题，并且在数值计算上更加稳健，
    可以显著提高贝叶斯优化的性能。
...
class LogConstrainedExpectedImprovement(AnalyticAcquisitionFunction):
    ...
    def __init__(
        ...
        适用于多输出模型。计算在考虑可行性约束的情况下的目标函数的对数预期改进（Log Constrained Expected Improvement，LogCEI）。
        Args:
...
class ConstrainedExpectedImprovement(AnalyticAcquisitionFunction):
    ...
    计算在考虑可行性约束的情况下的目标函数的预期改进（Expected Improvement，EI）
    将预期改进（EI）与每个输出的可行性概率相乘来计算加权的预期改进。

    在贝叶斯优化框架中，可行性约束可以通过概率的方式来处理，即计算在给定的约束边界内的概率。
    例如，如果我们有一个约束 y_i 必须位于 [lower_i, upper_i] 之间，
    我们可以计算 y_i 落在这个区间内的概率，并将其作为可行性权重因子。
    在 ConstrainedExpectedImprovement 类中，这个概念被用来计算加权的预期改进值，
    其中每个输出的预期改进值都乘以该输出满足约束的概率。
